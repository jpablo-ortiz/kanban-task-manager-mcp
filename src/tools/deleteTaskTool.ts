...
export const deleteTaskTool = (
  ...
  const processRequest = async (
    ...
    logger.info(
      `[${TOOL_NAME}] Received request to delete ${args.task_ids.length} tasks from project ${args.project_id}`
    );
    try {
      // Call the service method to delete the tasks
      const deletedCount = await taskService.deleteTasks(
        args.project_id,
        args.task_ids
      );
      ...
      logger.info(
        `[${TOOL_NAME}] Successfully deleted ${deletedCount} tasks from project ${args.project_id}`
      );
      return {
        content: [
          {
      ...
      if (error instanceof NotFoundError) {
        ...
        const message =
          error instanceof Error
            ? error.message
            : "An unknown error occurred while deleting tasks.";
