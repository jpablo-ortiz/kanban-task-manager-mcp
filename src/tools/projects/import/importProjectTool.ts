 ...
 */
export const importProjectTool = (
  server: McpServer,
  projectService: ProjectService
): void => {
  const processRequest = async (args: ImportProjectArgs) => {
    logger.info(
      `[${TOOL_NAME}] Received request (project name: ${args.new_project_name || "Default"})`
    );
    try {
      ...
      logger.info(
        `[${TOOL_NAME}] Successfully imported project. New ID: ${result.project_id}`
      );
      return {
        content: [
          {
      ...
      if (error instanceof ValidationError) {
        ...
        const message =
          error instanceof Error
            ? error.message
            : "An unknown error occurred during project import.";
