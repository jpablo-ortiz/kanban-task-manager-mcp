...
import {
  ...
  TOOL_PARAMS,
  DeleteProjectArgs,
} from "./deleteProjectParams.js";
import { ProjectService } from "../../../services/index.js"; // Assuming ProjectService is exported from index
...
export const deleteProjectTool = (
  ...
  const processRequest = async (
    ...
    logger.warn(
      `[${TOOL_NAME}] Received request to DELETE project ${args.project_id}. This is a destructive operation.`
    ); // Log deletion intent clearly
    try {
      ...
      logger.info(
        `[${TOOL_NAME}] Successfully deleted project ${args.project_id}`
      );
      return {
        content: [
          {
