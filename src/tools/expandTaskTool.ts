...
export const expandTaskTool = (
  ...
  const processRequest = async (args: ExpandTaskArgs) => {
    ...
    try {
      ...
      logger.info(
        `[${TOOL_NAME}] Successfully expanded task ${args.task_id} in project ${args.project_id}`
      );
      return {
        content: [
          {
      ...
      if (error instanceof NotFoundError) {
        ...
        const message =
          error instanceof Error
            ? error.message
            : "An unknown error occurred while expanding the task.";
