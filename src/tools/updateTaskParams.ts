...
export const UPDATE_TASK_BASE_SCHEMA = z.object({
  project_id: z
    ...
    .describe(
      "The unique identifier (UUID) of the project containing the task to update. This project must exist."
  ...
  task_id: z
    ...
    .describe(
      "The unique identifier (UUID) of the task to update. This task must exist within the specified project."
    ), // Required, UUID format
  ...
  description: z
    ...
    .optional()
    .describe(
      "Optional. The new textual description for the task (1-1024 characters)."
  ...
  priority: z
    ...
    .optional()
    .describe(
      "Optional. The new priority level for the task ('high', 'medium', or 'low')."
    ), // Optional, enum
  ...
  dependencies: z
    ...
    .optional()
    .describe(
      "Optional. The complete list of task IDs (UUIDs) that this task depends on. Replaces the existing list entirely. Max 50 dependencies."
    ), // Optional, array of UUID strings, limit
});
...
export const TOOL_PARAMS = UPDATE_TASK_BASE_SCHEMA.refine(
  ...
  {
    message:
      "At least one field to update (description, priority, or dependencies) must be provided.",
