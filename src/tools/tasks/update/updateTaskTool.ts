...
export const updateTaskTool = (
  ...
  const processRequest = async (
    ...
    logger.info(`[${TOOL_NAME}] Received request with args:`, {
      ...args,
      dependencies: args.dependencies
        ? `[${args.dependencies.length} items]`
        : undefined,
    }); // Avoid logging potentially large arrays
    try {
      ...
      logger.info(
        `[${TOOL_NAME}] Successfully updated task ${args.task_id} in project ${args.project_id}`
      );
      return {
        content: [
          {
