...
import {
  projectImportSchema,
  ValidatedProjectImportData,
} from "../types/projectImportSchema.js";
...
export class ProjectService {
  ...
  public async importProject(
    ...
    if (!validationResult.success) {
      ...
      logger.warn(
        "[ProjectService] Import data validation failed:",
        errorMessages
      );
      throw new ValidationError(
        `Invalid project data format: ${errorMessages.join("; ")}`
      );
    }

    const importData: ValidatedProjectImportData = validationResult.data;
    logger.debug(
      `[ProjectService] Successfully parsed and validated import data.`
    );
    ...
    const importTransaction = this.db.transaction(() => {
      ...
      const finalProjectName =
        newProjectName?.trim() ||
        `${importData.project_metadata.name} (Imported ${now})`; // Name from validated data
      const newProjectData: ProjectData = {
        // Renamed to avoid conflict with 'newProject' outer scope var
      ...
      const processTask = (
        taskFromFile: ValidatedProjectImportData["tasks"][number],
        ...
        if (taskFromFile.subtasks && taskFromFile.subtasks.length > 0) {
          taskFromFile.subtasks.forEach((subtask: any) =>
            processTask(subtask, newTaskId)
          );
        }
      };
      ...
      logger.info(
        `[ProjectService] Processed ${idMap.size} tasks for import (creation phase).`
      );
      ...
      const processDepsRecursively = (
        ...
        if (
          ...
          for (const oldDependencyId of taskFromFile.dependencies) {
            ...
            if (newDependencyId) {
              if (newCurrentTaskId === newDependencyId) {
                // Check for self-dependency with new IDs
                logger.warn(
                  `[ProjectService] Skipping self-dependency for task ${taskFromFile.description} (Old ID: ${taskFromFile.task_id})`
                );
                continue;
              }
              ...
              logger.warn(
                `[ProjectService] Dependency task with old ID ${oldDependencyId} not found in ID map for task ${taskFromFile.description}. Skipping dependency.`
              );
            }
          }
        }
    ...
    try {
      ...
      if (error instanceof RepositoryNotFoundError) {
        ...
        throw new ValidationError(
          `Data integrity issue during import: ${error.message}`
        );
      } else if (
  ...
  public async updateProject(
    ...
    // Corrected: ProjectData does not have updated_at
    logger.info(
      { projectId, newName: projectName },
      "Service attempting to update project name"
    );
    ...
    if (currentProject.name === trimmedProjectName) {
      logger.info(
        `Project name for ${projectId} is already '${trimmedProjectName}'. No update performed.`
      );
    ...
    try {
      ...
      logger.info(
        ...
        "Project updated successfully by service"
      );
      ...
      if (error instanceof RepositoryNotFoundError) {
        ...
        logger.warn(
          { error, projectId },
          "Update failed: Project not found or name unchanged at repository level."
        );
      ...
      logger.error(
        { error, projectId, newName: trimmedProjectName },
        "Error updating project in service"
      );
